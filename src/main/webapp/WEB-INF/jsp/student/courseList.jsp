                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <form class="bs-example bs-example-form col-md-5" role="form" style="margin: 20px 0 10px 0;"
                              action="${pageContext.request.getContextPath()}/admin/selectCourse" id="form1"
                              method="post">
                ...
                <div class="panel-footer">
                    <c:if test="${pageBean != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li>
                                    <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.currentPage-1}">&laquo;上一页</a>
                                </li>
                                <li class="active"><a href="">${pageBean.currentPage}</a></li>
                                <c:if test="${pageBean.currentPage+1 <= pageBean.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.currentPage+1}">${pageBean.currentPage+1}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pageBean.currentPage+2 <= pageBean.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.currentPage+2}">${pageBean.currentPage+2}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pageBean.currentPage+3 <= pageBean.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.currentPage+3}">${pageBean.currentPage+3}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pageBean.currentPage+4 <= pageBean.totalPage}">
                                    <li>
                                        <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.currentPage+4}">${pageBean.currentPage+4}</a>
                                    </li>
                                </c:if>
                                <li>
                                    <a href="${pageContext.request.getContextPath()}/student/course_list/${pageBean.totalPage}">最后一页&raquo;</a>
                                </li>
                            </ul>
                        </nav>
                    </c:if>
